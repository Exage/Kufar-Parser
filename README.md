...
Фильтр по названию находится в `parser/filters.py`, а конкретные слова задаются в `config/categories.json`.

## Что делает

- Читает правила из `config/categories.json`
- Парсит объявления с пагинацией
- Фильтрует по цене, `titleKeywords`, `titleExcludeKeywords`, `excludeKeywords`
...
- `titleKeywords`: список вариантов, один из которых обязательно должен быть в названии объявления. Например, `["9070 xt", "9070xt"]`
- `titleExcludeKeywords`: список точечных исключений по названию. Например, чтобы правило `9070` не ловило `9070 XT`
- `keywords`: старое имя include-фильтра; работает как `titleKeywords`, если `titleKeywords` не задан
- `excludeKeywords`: список общих исключаемых слов по названию
